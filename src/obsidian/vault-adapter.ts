...
export class ObsidianVault implements LocalVault {
	...
	async write(path: string, content: string, expected?: string): Promise<boolean> {
		...
		// Vault.process reads and writes under one lock. Doing this as read-then-
		// modify would let a keystroke land in between and be overwritten.
		let wrote = true;
	...
	async resolveEmbed(name: string, fromNotePath: string): Promise<string | null> {
		// Defer to Obsidian so an embed resolves exactly as it does in the editor,
		// including its shortest-path and same-folder preferences.
		return this.app.metadataCache.getFirstLinkpathDest(name, fromNotePath)?.path ?? null;
	}
	...
	private requireFile(path: string): TFile {
		const file = this.app.vault.getFileByPath(normalizePath(path));
		if (!file) throw new Error(`No such note in the vault: ${path}`);
		return file;
	}
